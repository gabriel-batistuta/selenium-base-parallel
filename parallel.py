...
if __name__ == "__main__":
	...
	# Número de threads que deseja utilizar (geralmente, o número de núcleos da CPU)
	num_threads = 4

	# Marca o início da execução paralela
	total_start_time = time.time()

	# Cria um executor para gerenciar o pool de threads
	with ThreadPoolExecutor(max_workers=num_threads) as executor:
		times = list(executor.map(fetch_page, urls))

	# Marca o fim da execução paralela
	total_end_time = time.time()
	total_execution_time = total_end_time - total_start_time

	print(f"\nTempo total de execução paralela: {total_execution_time:.2f} segundos")
